...
parser.add_argument('--output_dir', default='/data1/home/yuanzhouhang/Result/MAGE-B/generation', type=str,
                    help='name')
parser.add_argument('--peft', default=False, type=bool,)
parser.add_argument('--vae', default='no', type=str,)
...
if not args.peft:
    ...
    if args.vae is not None:
        model = models_mage_peft_vae.__dict__[args.model](norm_pix_loss=False,
                                             mask_ratio_mu=0.55, mask_ratio_std=0.25,
                                             mask_ratio_min=0.0, mask_ratio_max=1.0,
                                             vqgan_ckpt_path=args.vqgan_ckpt_path, vae_set=args.vae)
    else:
        model = models_mage_peft.__dict__[args.model](norm_pix_loss=False,
                                                      mask_ratio_mu=0.55, mask_ratio_std=0.25,
                                                      mask_ratio_min=0.0, mask_ratio_max=1.0,
                                                      vqgan_ckpt_path=args.vqgan_ckpt_path)
